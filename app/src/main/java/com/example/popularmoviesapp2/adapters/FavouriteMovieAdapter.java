...
public class FavouriteMovieAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public FavouriteMovieAdapter(Context context, ArrayList<FavouriteMovieData> movieList) {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, int position) {
        ...
        final FavouriteMovieData favouriteMovieData = movieList.get(position);

        movieHolder.tvTitle.setText(favouriteMovieData.getTitle());
        Picasso.get().load((BuildConfig.POSTER_BASE_URL + favouriteMovieData.getPoster_path()).trim())
                .placeholder(R.drawable.placeholder_image)
                .into(movieHolder.imgPoster);

        SimpleDateFormat simpleDateFormat = new SimpleDateFormat("yyyy-MM-dd", Locale.ENGLISH);
        try {
            Date date = simpleDateFormat.parse(favouriteMovieData.getRelease_date());
        ...
        movieHolder.imgPoster.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                adapterListener.moveToDetailsScreen(movieHolder.imgPoster, favouriteMovieData);
            }
        });
